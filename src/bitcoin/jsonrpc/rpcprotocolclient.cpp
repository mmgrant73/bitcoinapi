...
namespace jsonrpc
{
    ...
    void RpcProtocolClient::HandleResponse(const std::string &response, Json::Value& result) throw(JsonRpcException)
    {
        ...
        {
	    std::string serror=value["error"].asString();
            if(value.isMember(KEY_ID) && value.isMember(KEY_RESULT) && value.isMember(KEY_ERROR) && serror=="")
            {
	    ...
	    else if(value.isMember(KEY_ID) && value.isMember(KEY_RESULT) && value.isMember(KEY_ERROR) && serror!="")
	    {
		throw JsonRpcException(value[KEY_ERROR][KEY_ERROR_CODE].asInt());
	    }
            else
            {
